...
import { AnalysisType, AIAnalysisResults, AIAnalysisMetadata } from '../src/lib/types'
...
export type AnalysisQueueStatus = 'pending' | 'processing' | 'completed' | 'failed' | 'retrying'
...
export const queueAnalysisForJournalEntry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newAnalysisTypes = args.analysisTypes.filter(type => !existingTypes.has(type))
    ...
    const scheduledId = await ctx.scheduler.runAfter(0, 'aiAnalysis:processAnalysisQueue' as any, {
    ...
    try {
      ...
      for (const analysisType of args.analysisTypes) {
        try {
          ...
          switch (analysisType) {
            case 'sentiment':
              analysisResult = await analyzer.analyzeSentiment(journalEntry.content)
              break
            case 'emotional_stability':
              // Get recent sentiment history for stability analysis
              const sentimentHistory = await ctx.runQuery('aiAnalysis:getSentimentTrends' as any, {
                relationshipId: args.relationshipId,
                days: 30
              })
              analysisResult = await analyzer.analyzeEmotionalStability(sentimentHistory)
              break
            case 'energy_impact':
              analysisResult = await analyzer.analyzeEnergyImpact(journalEntry.content)
              break
            default:
          ...
          const analysisId = await ctx.runMutation('aiAnalysis:createAnalysis' as any, {
            journalEntryId: args.journalEntryId,
            relationshipId: args.relationshipId,
            userId: args.userId,
            analysisType,
            analysisResults: {
              sentimentScore: analysisResult.sentiment_score || analysisResult.sentimentScore,
              emotions: analysisResult.emotions_detected || analysisResult.emotions || [],
          ...
          if (args.analysisTypes.indexOf(analysisType) < args.analysisTypes.length - 1) {
          ...
          if (retryAttempt <= RATE_LIMIT_CONFIG.retryAttempts) {
            const retryDelay = RATE_LIMIT_CONFIG.retryDelayMs * Math.pow(2, retryAttempt - 1)
            
            await ctx.scheduler.runAfter(retryDelay, 'aiAnalysis:processAnalysisQueue' as any, {
              ...args,
      ...
      throw new Error(`Analysis processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
})
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      for (const journalEntryId of batch) {
        const scheduledId = await ctx.scheduler.runAfter(delay, 'aiAnalysis:queueAnalysisForJournalEntry' as any, {
          journalEntryId,
