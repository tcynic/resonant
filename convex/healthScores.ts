...
export const getHealthScoresByUser = query({
  args: {
    userId: v.id('users'),
    sortBy: v.optional(v.union(v.literal('score'), v.literal('updated'), v.literal('relationship')))
  },
  handler: async (ctx, args) => {
...
export const calculateHealthScoreFromAnalyses = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const componentScores: HealthScoreComponents = {
      ...
      emotionalStability: calculateStabilityScore(analysesByType.emotional_stability || []),
      energyImpact: calculateEnergyScore(analysesByType.energy_impact || []),
      conflictResolution: calculateResolutionScore(analysesByType.conflict_resolution || []),
    ...
    const overallScore = Math.round(
      ...
      (componentScores.communicationFrequency * SCORE_WEIGHTS.communicationFrequency)
    )
    ...
    const trendsData = await calculateTrends(ctx, args.relationshipId, overallScore)
...
function calculateStabilityScore(stabilityAnalyses: any[]): number {
  ...
  const scores = stabilityAnalyses.map(analysis => analysis.analysisResults.stabilityScore || 50)
...
function calculateEnergyScore(energyAnalyses: any[]): number {
  ...
  const scores = energyAnalyses.map(analysis => (analysis.analysisResults.energyScore || 5) * 10)
  const average = scores.reduce((sum, score) => sum + score, 0) / scores.length
...
function calculateResolutionScore(resolutionAnalyses: any[]): number {
  ...
  // Convert 1-10 scale to 0-100 scale
  const scores = resolutionAnalyses.map(analysis => (analysis.analysisResults.resolutionScore || 5) * 10)
  const average = scores.reduce((sum, score) => sum + score, 0) / scores.length
...
function calculateGratitudeScore(gratitudeAnalyses: any[]): number {
  ...
  // Convert 1-10 scale to 0-100 scale
  const scores = gratitudeAnalyses.map(analysis => (analysis.analysisResults.gratitudeScore || 5) * 10)
  const average = scores.reduce((sum, score) => sum + score, 0) / scores.length
...
function calculateCommunicationScore(allAnalyses: any[], timeRangeDays: number): number {
...
function calculateConfidenceLevel(analyses: any[], componentScores: HealthScoreComponents): number {
  ...
  const avgAIConfidence = analyses.reduce((sum, analysis) => {
    return sum + (analysis.analysisResults.confidence || 0.5)
  }, 0) / analyses.length
  
  // Penalize if too many components are at default (50) score
  const defaultComponents = Object.values(componentScores).filter(score => score === 50).length
  const componentPenalty = Math.max(0, 1 - (defaultComponents * 0.15))
  
  const overallConfidence = dataAvailability * avgAIConfidence * componentPenalty
  
  return Math.min(1, Math.max(0.1, overallConfidence))
}

// Helper function: Calculate trends
async function calculateTrends(ctx: any, relationshipId: Id<'relationships'>, currentScore: number): Promise<HealthScoreTrends | undefined> {
  // Get previous health score for comparison
  const previousScore = await ctx.db
    .query('healthScores')
    .withIndex('by_relationship', (q: any) => q.eq('relationshipId', relationshipId))
    .first()
  
  if (!previousScore) return undefined
  
  const scoreDifference = currentScore - previousScore.overallScore
  const changeRate = Math.abs(scoreDifference) / previousScore.overallScore * 100
...
export const getHealthScoreTrends = query({
  ...
  handler: async (ctx, args) => {
    ...
    const weeklyTrends = sentimentAnalyses.reduce((acc, analysis) => {
      const weekStart = Math.floor(analysis.createdAt / (7 * 24 * 60 * 60 * 1000)) * (7 * 24 * 60 * 60 * 1000)
      ...
      if (analysis.analysisResults.sentimentScore) {
        acc[weekStart].scores.push(analysis.analysisResults.sentimentScore * 10) // Convert to 0-100
        acc[weekStart].count++
      }
      
      return acc
    }, {} as Record<number, { timestamp: number; scores: number[]; count: number }>)
    
    // Calculate weekly averages
    const trends = Object.values(weeklyTrends)
      .map(week => ({
        timestamp: week.timestamp,
        averageScore: week.scores.reduce((sum, score) => sum + score, 0) / week.scores.length,
...
export const recalculateAllHealthScores = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const relationship of relationships) {
      try {
        const result = await ctx.runMutation('healthScores:calculateHealthScoreFromAnalyses' as any, {
