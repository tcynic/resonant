...
export const withMetrics = <T extends any[], R>(
  ...
  return async (...args: T): Promise<R> => {
    ...
    try {
      ...
      await trackConvexMetric(ctx, `function.duration.${functionName}`, Date.now() - startTime);
...
export const userEngagementMetrics = {
  ...
  trackRelationshipAction: (action: 'create' | 'update' | 'delete', userId: string, relationshipType: string) => {
...
export const withDbMetrics = <T>(queryName: string, queryFn: () => Promise<T>) => {
...
async function sendAlert(alert: any) {
  ...
  if (config.channels.includes('slack-critical') || config.channels.includes('slack-alerts')) {
...
async function sendToMonitoringService(metric: MetricData & { tags: Record<string, string> }) {
...
export const getRecentMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const aiLatency = await getAverageMetric(
      ctx,
      "ai_analysis.latency",
      since
    );
...
async function aggregateMetricsByMinute(ctx: any, metricName: string, since: number) {
...
async function aggregateMetricsByHour(ctx: any, metricName: string, since: number) {
  const metrics = await ctx.db
    .query("metrics")
    .filter(q => q.and(
      q.eq(q.field("name"), metricName),
...
export const createIncident = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.incidents.triggerIncidentResponse, {
      incidentId,
      severity: args.severity,
    });
...
export const triggerIncidentResponse = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(escalationTime, internal.incidents.escalateIncident, {
...
async function sendIncidentNotification(incident: any, config: any) {
  ...
  console.log(`Sending ${config.description} notification for incident ${incident.id}`);
}
