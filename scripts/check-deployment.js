...
async function checkDeployment() {
  ...
  try {
    ...
    if (text.includes('Application error: a client-side exception has occurred')) {
      ...
      console.log('   - Wait for deployment to complete (check Vercel dashboard)')
      console.log('   - Verify NEXT_PUBLIC_CONVEX_URL is set for Production environment')
      console.log('   - Try a hard refresh of the deployment')
      return false
    } else if (text.includes('<title>') && !text.includes('Application error')) {
...
async function main() {
  ...
  if (isWorking) {
    ...
    console.log('🔄 If deployment is still in progress, wait 2-3 minutes and run again')
