          ...
          if (healthData.checks) {
            Object.entries(healthData.checks).forEach(([service, check]) => {
              if (check.status === 'healthy') {
                ...
                this.logError(
                  `${service}: ${check.status} - ${check.error || 'Unknown error'}`
                )
              }
            })
          }
  ...
  async checkDatabaseConnectivity() {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        this.logWarning(
          'Database test endpoint not found (this is expected in production)'
        )
      } else {
      ...
      this.logWarning(
        `Database connectivity check inconclusive: ${error.message}`
      )
    }
  }
  ...
  async checkAuthenticationService() {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        this.logError(
          `Authentication page returned status ${response.statusCode}`
        )
      }
    } catch (error) {
  ...
  async checkPerformance() {
    ...
    try {
      ...
      if (headers['content-encoding']) {
        this.logSuccess(
          `Content compression active: ${headers['content-encoding']}`
        )
      } else {
