...
class TestPerformanceMonitor {
  ...
  calculateParallelizationEfficiency(testResults) {
    ...
    const serialDuration =
      testResults.serialDuration || testResults.totalDuration
  ...
  detectFlakyTests(testResults) {
    ...
    return {
      ...
      tests: flakyTests,
      totalFlakinessScore: flakyTests.reduce(
        (sum, test) => sum + test.flakinessScore,
        0
      ),
    }
  }
  ...
  async calculateTrends() {
    ...
    return {
      ...
      executionTime: this.calculateTrend(
        recent.map(h => h.execution.totalDuration)
      ),
  ...
  async analyzePerformance(metrics) {
    ...
    if (
      metrics.execution.averageTestDuration > this.thresholds.avgTestDuration
    ) {
      issues.push({
        type: 'warning',
        category: 'execution',
      ...
      recommendations.push(
        'Consider optimizing slow tests or increasing parallelization'
      )
    }
    ...
    if (metrics.resource.memory.peak > this.thresholds.memoryUsage) {
      ...
      recommendations.push(
        'Investigate memory leaks in test setup or application code'
      )
    }
    ...
    if (metrics.reliability.errorRate > this.thresholds.errorRate) {
      ...
      recommendations.push(
        'Review and fix failing tests to improve reliability'
      )
    }
    ...
    if (metrics.reliability.flakiness.count > 0) {
      ...
      recommendations.push(
        'Investigate and stabilize flaky tests to improve consistency'
      )
    }
  ...
  scoreResource(resource) {
    ...
    const memoryScore =
      resource.memory.peak <= 256 ? 100 : resource.memory.peak <= 512 ? 80 : 60
    const cpuScore =
      resource.cpu.peak <= 50 ? 100 : resource.cpu.peak <= 70 ? 80 : 60
  ...
  scoreReliability(reliability) {
    ...
    return Math.max(
      0,
      100 - reliability.errorRate * 10 - reliability.timeoutRate * 5
    )
  }
  ...
  printAnalysisResults(report) {
    ...
    console.log(
      `  Overall Score: ${report.overall.overall}/100 (Grade: ${report.overall.grade})`
    )
