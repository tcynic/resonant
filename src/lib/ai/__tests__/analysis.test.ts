...
import { SentimentAnalysisModule, EmotionalStabilityModule, EnergyImpactModule, RelationshipAnalyzer } from '../analysis'
...
describe('Enhanced DSPy Analysis Modules', () => {
  ...
  describe('SentimentAnalysisModule', () => {
    test('should analyze positive sentiment correctly', async () => {
      const module = new SentimentAnalysisModule(mockGeminiClient)
      ...
      const result = await module.analyzeSentiment("I had a wonderful day with my partner. We laughed and felt so connected.")
      ...
      expect(mockGeminiClient.generateStructuredResponse).toHaveBeenCalledTimes(1)
    })
    ...
    test('should analyze negative sentiment correctly', async () => {
      ...
      const result = await module.analyzeSentiment("We had a big fight today. I feel so frustrated and hurt.")
    ...
    test('should generate DSPy-style prompts with examples', async () => {
      ...
      await module.analyzeSentiment("Today was okay with my partner.")

      // Verify the prompt includes DSPy examples and proper structure
      const promptCall = mockGeminiClient.generateStructuredResponse.mock.calls[0][0]
      expect(promptCall).toContain('Analyze sentiment of relationship journal entries')
    ...
    test('should validate inputs correctly', async () => {
      ...
      await expect(module.forward({ journal_entry: "Hi" }))
        .rejects.toThrow('AI response validation failed: Input validation failed')
    })
  })

  describe('EmotionalStabilityModule', () => {
    test('should analyze stability patterns correctly', async () => {
      const module = new EmotionalStabilityModule(mockGeminiClient)
      
      mockGeminiClient.generateStructuredResponse.mockResolvedValue({
        stability_score: 75,
        trend_direction: 'improving',
        volatility_level: 'low',
        recovery_patterns: 'Quick recovery from negative events, consistent positive trajectory'
      })
      ...
      const result = await module.analyzeStability(sentimentHistory)
    ...
    test('should generate stability analysis prompts with history', async () => {
      ...
      await module.analyzeStability(sentimentHistory)
  ...
  describe('EnergyImpactModule', () => {
    test('should analyze energizing interactions', async () => {
      const module = new EnergyImpactModule(mockGeminiClient)
      
      mockGeminiClient.generateStructuredResponse.mockResolvedValue({
        energy_score: 9,
        energy_indicators: ['excitement', 'motivation', 'joy'],
        overall_effect: 'energizing',
        explanation: 'High energy positive interaction with strong motivational elements'
      })

      const result = await module.analyzeEnergyImpact("I feel so energized after our conversation! We made exciting plans together.")
    ...
    test('should analyze draining interactions', async () => {
      ...
      mockGeminiClient.generateStructuredResponse.mockResolvedValue({
        ...
        explanation: 'Highly draining interaction with conflict and emotional exhaustion'
      })

      const result = await module.analyzeEnergyImpact("I'm completely drained after that argument. So much stress and negativity.")
  ...
  describe('RelationshipAnalyzer (DSPy Pipeline)', () => {
    test('should perform comprehensive multi-stage analysis', async () => {
      ...
      expect(result.metadata.modules_used).toEqual(['sentiment', 'energy', 'stability'])
    ...
    test('should calculate overall confidence correctly', async () => {
      ...
      const result = await analyzer.analyzeJournalEntry("Today was pretty normal.")
  ...
  describe('DSPy Integration Patterns', () => {
    test('should follow DSPy signature validation patterns', async () => {
      ...
      const signature = module.getSignature()
    ...
    test('should generate proper DSPy-style prompts', async () => {
      ...
      await module.analyzeSentiment("This is a test entry.")

      const prompt = mockGeminiClient.generateStructuredResponse.mock.calls[0][0]
      
      // Should include DSPy signature elements
      expect(prompt).toContain('Analyze sentiment of relationship journal entries')
