...
import { AIRateLimiter, withRateLimit } from '../rate-limiter'
import { AICostTracker, checkBudget } from '../cost-tracker'
...
describe('AI Rate Limiting and Cost Control', () => {
  ...
  describe('Cost Tracker', () => {
    ...
    test('should enforce analysis type limits', () => {
      ...
      expect(budgetCheck.allowed).toBe(false)
      expect(budgetCheck.reason).toContain(
        'Analysis type (sentiment) daily budget exceeded'
      )
    })
    ...
    test('should integrate with checkBudget helper', async () => {
      // Test the global checkBudget helper function
      await expect(
        checkBudget(
          'sentiment',
          100, // Exceeds budget
