...
export class AICostTracker {
  ...
  recordCost(record: Omit<CostRecord, 'id' | 'timestamp'>): string {
    ...
    console.debug(
      `💸 Recorded cost: $${(cost / 100).toFixed(4)} for ${record.analysisType}`
    )
  ...
  checkBudget(
    ...
    if (this.budgetState.dailySpent + estimatedCost > this.config.dailyBudget) {
      return {
        ...
        percentageUsed:
          (this.budgetState.dailySpent / this.config.dailyBudget) * 100,
      }
    }
  ...
  private checkBudgetAlerts(): void {
    ...
    this.checkThresholdAlert(
      'daily',
      dailyUsage,
      this.budgetState.dailySpent,
      this.config.dailyBudget
    )

    // Check weekly budget
    const weeklyUsage =
      (this.budgetState.weeklySpent / this.config.weeklyBudget) * 100
    this.checkThresholdAlert(
      'weekly',
      weeklyUsage,
      this.budgetState.weeklySpent,
      this.config.weeklyBudget
    )

    // Check monthly budget
    const monthlyUsage =
      (this.budgetState.monthlySpent / this.config.monthlyBudget) * 100
    this.checkThresholdAlert(
      'monthly',
      monthlyUsage,
      this.budgetState.monthlySpent,
  ...
  private getAnalysisTypeSpend(
    ...
    return this.costRecords
      .filter(
        record =>
          record.timestamp >= cutoff && record.analysisType === analysisType
      )
      .reduce(
        (sum, record) => sum + (record.actualCost || record.estimatedCost),
        0
      )
  }
  ...
  private getOrganizationSpend(
    ...
    return this.costRecords
      .filter(
        record =>
          record.timestamp >= cutoff && record.organizationId === organizationId
      )
      .reduce(
        (sum, record) => sum + (record.actualCost || record.estimatedCost),
        0
  ...
  generateCostSummary(period: 'day' | 'week' | 'month'): CostSummary {
    ...
    const totalCost = periodRecords.reduce(
      (sum, record) => sum + (record.actualCost || record.estimatedCost),
      0
    )
    ...
    for (const analysisType of [
      ...
      const typeRecords = periodRecords.filter(
      ...
      const typeTokens = typeRecords.reduce((sum, r) => sum + r.totalTokens, 0)
      ...
      byAnalysisType[analysisType] = {
        ...
        avgTokens: typeRecords.length > 0 ? typeTokens / typeRecords.length : 0,
      }
    }
  ...
  private cleanupOldRecords(): void {
    ...
    this.costRecords = this.costRecords.filter(
      record => record.timestamp >= cutoff
    )
...
export function getAICostTracker(
