...
import { HealthScoreComponents } from '../types'
...
class FallbackSentimentAnalyzer {
  ...
  analyzeSentiment(text: string): SentimentAnalysisResult {
    ...
    const relationshipPositiveImpact =
      (scores.relationshipPositive / totalWords) * 2
    const relationshipNegativeImpact =
      (scores.relationshipNegative / totalWords) * -2

    sentimentScore +=
      positiveImpact +
      negativeImpact +
      relationshipPositiveImpact +
      relationshipNegativeImpact
  ...
  private scoreWords(words: string[]): {
    ...
    let positive = 0,
      negative = 0,
      neutral = 0
    let relationshipPositive = 0,
      relationshipNegative = 0
  ...
  private calculateConfidence(
    ...
    const emotionalWords = scores.positive + scores.negative + scores.neutral
...
class FallbackHealthScoreCalculator {
  calculateBasicHealthScore(
    ...
    const avgSentiment =
      recentSentiments.reduce((sum, s) => sum + s.score, 0) /
...
export class AIFallbackService {
  ...
  async analyzeStabilityFallback(
    ...
    if (sentimentHistory.length < 2) {
      return {
        ...
        recovery_patterns:
          'Insufficient data for pattern analysis (offline mode)',
      }
    }
  ...
  async analyzeEnergyFallback(journalEntry: string): Promise<{
    ...
    const energyIndicators = sentiment.emotions_detected.map(
  ...
  async calculateHealthScoreFallback(relationshipData: {
    recentSentiments: Array<{ score: number; timestamp: number }>
    ...
    const recencyPenalty = Math.min(
      relationshipData.daysSinceLastEntry / 30,
      0.3
    )
...
export async function withFallback<T>(
  ...
  try {
    ...
    if (error instanceof AIError && aiFallback.shouldUseFallback(error)) {
      ...
      if (!aiFallback.isFallbackActive()) {
        aiFallback.activateFallback(
          `${errorContext} failure: ${error.constructor.name}`
        )
      }
