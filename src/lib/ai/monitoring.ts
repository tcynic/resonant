...
import { AIError, AIErrorSeverity, AIErrorStats, getErrorSeverity } from './errors'
...
interface AIAlert {
  ...
  type: 'error_rate' | 'failure_rate' | 'latency' | 'queue_depth' | 'service_down'
  message: string
  threshold: number
  currentValue: number
...
class AIMonitoringService {
  ...
  startTiming(operation: string, analysisType: AnalysisType, context: Record<string, unknown> = {}): () => void {
  ...
  getErrorStats(timeRangeMs: number = 3600000): AIErrorStats { // Default 1 hour
  ...
  getPerformanceMetrics(timeRangeMs: number = 3600000): PerformanceMetrics {
    const cutoff = Date.now() - timeRangeMs
    const recentMetrics = this.performanceWindow.filter(m => m.timestamp >= cutoff)
    ...
    return {
      ...
      queueMetrics: {
        averageDepth: recentMetrics.reduce((sum, m) => sum + (m.queueDepth || 0), 0) / recentMetrics.length,
  ...
  private checkAlertThresholds(): void {
    ...
    if (stats.errorRate > this.config.alertThresholds.errorRate) {
      this.createAlert('error_rate', AIErrorSeverity.HIGH, 
        `High error rate detected: ${stats.errorRate.toFixed(1)} errors/hour`,
        this.config.alertThresholds.errorRate,
        stats.errorRate,
        ['AI Analysis Service'],
        ['Investigate recent errors', 'Check API service status', 'Review error logs']
      )
    }

    // Failure rate alert
    const failureRate = (performance.failedRequests / performance.totalRequests) * 100
    if (failureRate > this.config.alertThresholds.failureRate) {
      this.createAlert('failure_rate', AIErrorSeverity.HIGH,
    ...
    if (performance.p95Latency > this.config.alertThresholds.latencyP95) {
      this.createAlert('latency', AIErrorSeverity.MEDIUM,
    ...
    // Queue depth alert
    if (performance.queueMetrics.maxDepth > this.config.alertThresholds.queueDepth) {
      this.createAlert('queue_depth', AIErrorSeverity.MEDIUM,
        ...
        ['AI Analysis Queue'],
        ['Increase processing capacity', 'Review queue configuration', 'Check for bottlenecks']
      )
    }
  }
  ...
  private sendExternalAlert(alert: AIAlert): void {
    ...
    console.warn(`🚨 AI ALERT [${alert.severity.toUpperCase()}]: ${alert.message}`)
  }

  // Write log with appropriate level
  private writeLog(level: string, message: string, data: Record<string, unknown> = {}): void {
  ...
  // Health check
  getHealthStatus(): { status: 'healthy' | 'degraded' | 'down', issues: string[] } {
    ...
    // Check failure rate
    const failureRate = performance.totalRequests > 0 
  ...
  private startCleanupTimer(): void {
    this.cleanupInterval = setInterval(() => {
      const errorCutoff = Date.now() - (this.config.errorRetentionDays * 24 * 60 * 60 * 1000)
      const metricsCutoff = Date.now() - (this.config.metricsRetentionDays * 24 * 60 * 60 * 1000)
    ...
    .catch(error => {
      if (error instanceof AIError) {
        ...
        const aiError = new (class extends AIError {
          getUserMessage() { return 'An unexpected error occurred during AI analysis.' }
          getRecoveryActions() { return ['Try again', 'Contact support if issue persists'] }
        })(
          error.message || 'Unknown error',
          'UNKNOWN_ERROR',
          { operation, analysisType, originalError: error.message, ...context }
        )
