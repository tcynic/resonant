...
export class AIRateLimiter {
  ...
  private createBucket(timestamp: number, windowSizeMs: number): RateLimitBucket {
    return {
      count: 0,
      tokens: 0,
      cost: 0,
  ...
  async checkRateLimit(
    ...
    const checks = [
      this.checkBucket(this.minuteBucket, this.config.requestsPerMinute, this.config.tokensPerMinute, this.config.costPerMinute, estimatedTokens, estimatedCost),
      this.checkBucket(this.hourBucket, this.config.requestsPerHour, this.config.tokensPerHour, this.config.costPerHour, estimatedTokens, estimatedCost),
      this.checkBucket(this.dayBucket, this.config.requestsPerDay, this.config.tokensPerDay, this.config.costPerDay, estimatedTokens, estimatedCost)
    ]
    ...
    if (restrictiveCheck) {
      ...
      aiMonitoring.logError(
        new AIServiceRateLimitError('gemini', restrictiveCheck.retryAfter || 60000),
        {
          analysisType,
          estimatedTokens,
          estimatedCost,
          ...
          currentLimits: {
            minute: this.getBucketStatus(this.minuteBucket, this.config.requestsPerMinute, this.config.tokensPerMinute, this.config.costPerMinute),
            hour: this.getBucketStatus(this.hourBucket, this.config.requestsPerHour, this.config.tokensPerHour, this.config.costPerHour),
            day: this.getBucketStatus(this.dayBucket, this.config.requestsPerDay, this.config.tokensPerDay, this.config.costPerDay)
          }
        }
      )
  ...
  private checkBucket(
    ...
    const burstAllowance = bucket.windowSizeMs <= 60 * 1000 ? this.config.burstMultiplier : 1
    ...
    if (bucket.count + 1 > effectiveRequestLimit) {
      return {
        ...
        current: { requests: bucket.count, tokens: bucket.tokens, cost: bucket.cost },
        limits: { requests: effectiveRequestLimit, tokens: effectiveTokenLimit, cost: effectiveCostLimit },
  ...
  private getBucketStatus(bucket: RateLimitBucket, requestLimit: number, tokenLimit: number, costLimit: number) {
    return {
      requests: `${bucket.count}/${requestLimit}`,
      tokens: `${bucket.tokens}/${tokenLimit}`,
      cost: `$${(bucket.cost/100).toFixed(2)}/$${(costLimit/100).toFixed(2)}`,
      resetTime: new Date(bucket.windowStart + bucket.windowSizeMs).toISOString()
    }
  }
  ...
  private async queueRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (process.env.NODE_ENV !== 'test') {
        console.info(`📋 Queued request ${queueItem.id} (queue size: ${this.requestQueue.length})`)
      }
      ...
      setTimeout(() => {
        ...
        if (index !== -1) {
          ...
          reject(new AIResourceLimitError('requests', this.requestQueue.length, this.config.maxQueueSize, {
  ...
  private processQueue(): void {
    ...
    for (const queueItem of this.requestQueue) {
      ...
      if (status.allowed) {
        ...
        if (process.env.NODE_ENV !== 'test') {
          console.info(`✅ Processed queued request ${queueItem.id} after ${waitTime}ms`)
        }
    ...
    this.requestQueue = this.requestQueue.filter(item => !processed.includes(item.id))
  }
  ...
  getStatus(): {
    ...
    current: {
      minute: { requests: string; tokens: string; cost: string; resetTime: string }
      hour: { requests: string; tokens: string; cost: string; resetTime: string }
  ...
  clearQueue(): void {
    ...
    this.requestQueue.forEach(item => {
      item.reject(new AIResourceLimitError('requests', this.requestQueue.length, this.config.maxQueueSize, {
...
export async function withRateLimit<T>(
  ...
  // Check rate limit
  const status = await rateLimiter.checkRateLimit(analysisType, estimatedTokens, priority)
