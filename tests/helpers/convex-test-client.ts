...
export class ConvexTestClient {
   ...
   */
  async createTestUser(clerkId: string, name: string, email: string): Promise<Id<'users'>> {
    try {
      const userId = await this.client.mutation(api.test.testDataManager.createTestUser, {
        clerkId,
        name,
        email,
      })
  ...
  async createTestRelationships(
    ...
    try {
      ...
      console.log(`✅ Created ${relationships.length} relationships for user ${userId}`)
      return relationshipIds
    } catch (error) {
      console.error(`❌ Failed to create relationships for user ${userId}:`, error)
      throw error
    }
  }
  ...
  async createTestJournalEntries(
    ...
    try {
      ...
      console.log(`✅ Created ${entries.length} journal entries for user ${userId}`)
      return entryIds
    } catch (error) {
      console.error(`❌ Failed to create journal entries for user ${userId}:`, error)
      throw error
    }
  }
  ...
  async cleanupTestUser(clerkId: string): Promise<boolean> {
    try {
      ...
      if (result.deleted) {
        console.log(`✅ Cleaned up test user ${clerkId} (deleted ${result.deletedCount} records)`)
        return true
      } else {
        console.log(`ℹ️  Test user ${clerkId} not found for cleanup: ${result.reason}`)
        return false
      }
    } catch (error) {
  ...
  async cleanupAllTestData(): Promise<boolean> {
    try {
      ...
      console.log(`✅ Cleaned up all test data (${result.testUsersFound} users, ${result.deletedCount} total records)`)
      return true
    } catch (error) {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      console.log(`✅ Convex health check passed - ${stats.testUsers} test users found`)
      return true
    } catch (error) {
