...
test.describe('Smoke Tests - API Health', () => {
  test('Convex connection is healthy', async ({ page }) => {
    ...
    const criticalErrors = convexErrors.filter(
      error =>
  ...
  test('network requests complete without critical failures', async ({
    page,
  }) => {
  ...
  test('database schema compatibility', async ({ page }) => {
    ...
    const schemaErrors = consoleErrors.filter(
      error => error.includes('schema') || error.includes('validation')
    )
